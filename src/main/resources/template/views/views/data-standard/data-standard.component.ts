...
import { DataStandardListComponent } from '../../components/index';
import { UUID } from '@mt-framework-ng/util';

@Component({
  selector: 'app-data-standard',
  templateUrl: './data-standard.component.html',
  styleUrls: ['./data-standard.component.less']
})
export class DataStandardComponent implements OnInit {

  @ViewChild('dataStandardList', { static: false })
  dataStandardList: DataStandardListComponent;
  ...
  treeOption: TreeOption = {
    /** 目录树类型（报表管理） */
    treeCodeType: 'DATA_STANDARD',
    /** 是否开启非目录节点拖拽功能 */
    isDraggable: false,
    /** 树节点单击事件 */
    click: (node: NzTreeNode) => {
      this.category = node && <any>node.origin;
      if (this.dataStandardList) {
        this.dataStandardList.category = this.category;
        this.dataStandardList.queryOption.page = 0;
        this.dataStandardList.load();
      }
    },
    /** 树节点双击事件 */
  ...
  constructor(
    ...
    ngOnInit() {
      this.categoryTreeService.get('DATA_STANDARD').subscribe(data => {
        if (data && data.length) {
          // data[0]['nodeType'] = 'expand';
          this.nodes = TreeUtil.populateTreeNodes(data, 'name', 'id', 'children');
          if (this.dataStandardList) {
            this.dataStandardList.category = data[0];
            this.dataStandardList.queryOption.page = 0;
            this.dataStandardList.load();
          }
        }
